...
- **Key Concepts**:
  ...
  - **yt-dlp Search & Metadata Helpers**: Two module-level functions replace all `play-dl` YouTube HTTP calls to prevent 429 rate-limit errors on cloud IPs:
    ...
    - `ytdlpVideoInfo(url)` — runs `yt-dlp <url> --dump-json --no-playlist` and returns `{ title, url, duration }`. Used by `/play` when given a direct YouTube video URL. Does **not** pass `player_skip` but forces `youtube:player_client=ios;formats=missing_pot` to bypass GVS PO Token enforcement. It intentionally omits cookies to avoid skipping the `ios` client. Also collects stderr and logs it on non-zero exit for debuggability.
...
- `--extractor-args "youtube:player_skip=webpage,configs"` is passed **only to `ytdlpSearch`** (flat-playlist search). It is intentionally omitted from `ytdlpVideoInfo` and `playNext()`, which instead force the iOS player client (`youtube:player_client=ios;formats=missing_pot`) to bypass GVS PO Token enforcement and JavaScript n-challenge requirements.
- Cookies (`cookies.txt` / `YOUTUBE_COOKIES` env var) are passed **only to `ytdlpSearch` and `loadYtPlaylist`** (flat search/playlist listing). They are intentionally omitted from `ytdlpVideoInfo` and `playNext()` because the iOS player client does not support cookies, and passing them causes `yt-dlp` to skip the iOS client entirely, leading to errors.
