...
- **Key Concepts**:
  ...
  - **yt-dlp Search & Metadata Helpers**: Two module-level functions replace all `play-dl` YouTube HTTP calls to prevent 429 rate-limit errors on cloud IPs:
    ...
    - `ytdlpVideoInfo(url)` — runs `yt-dlp <url> --dump-json --no-playlist` and returns `{ title, url, duration }`. Used by `/play` when given a direct YouTube video URL. Does **not** pass `player_skip` but forces `youtube:player_client=ios,web,android;formats=missing_pot` to bypass GVS PO Token enforcement. It automatically passes `--cookies cookies.txt` (if present) to fall back to the `web` or `android` clients (and solves the n-challenge using the container's Node.js runtime) if the `ios` client gets blocked. Also collects stderr and logs it on non-zero exit for debuggability.
...
- `--extractor-args "youtube:player_skip=webpage,configs"` is passed **only to `ytdlpSearch`** (flat-playlist search). It is intentionally omitted from `ytdlpVideoInfo` and `playNext()`, which instead force the player client fallback list (`youtube:player_client=ios,web,android;formats=missing_pot`) to bypass GVS PO Token enforcement.
- Cookies (`cookies.txt` / `YOUTUBE_COOKIES` env var) are passed to `ytdlpSearch`, `loadYtPlaylist`, `ytdlpVideoInfo`, and `playNext()`. Node.js is installed in the Docker container to act as a JavaScript runtime. Crucially, `yt-dlp` is installed via `pip3 install "yt-dlp[default]"` (rather than downloading the standalone binary, which lacks EJS script bindings) so that `yt-dlp` can solve the n-challenge automatically using the Node.js runtime on the `web` or `android` fallback clients if the `ios` client gets blocked (e.g. "Sign in to confirm you're not a bot" errors).
