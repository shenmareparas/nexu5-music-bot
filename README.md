...
> [!NOTE]
> All YouTube interactions — including **search, metadata lookups, and audio streaming** — are routed through `yt-dlp`. `--extractor-args "youtube:player_skip=webpage,configs"` is applied **only** to `ytdlpSearch` (flat-playlist search) where no stream URL resolution is needed. For `ytdlpVideoInfo` and the streaming invocation in `playNext()`, the bot forces `--extractor-args youtube:player_client=ios,web,android;formats=missing_pot` to bypass GVS PO Token enforcement. Cookies are passed to all commands to ensure bypass of "Sign in to confirm you're not a bot" checks; the container includes `nodejs` to act as a JavaScript runtime and installs `yt-dlp` via python pip `yt-dlp[default]` so that `yt-dlp` includes the necessary Embedded JavaScript (EJS) helper scripts to automatically solve the n-challenge on the `web`/`android` fallback clients if the `ios` client gets skipped. `play-dl` is retained only for Spotify URL parsing (`play.spotify()`).
