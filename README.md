...
> [!NOTE]
> All YouTube interactions — including **search, metadata lookups, and audio streaming** — are routed through `yt-dlp`. `--extractor-args "youtube:player_skip=webpage,configs"` is applied **only** to `ytdlpSearch` (flat-playlist search) where no stream URL resolution is needed. For `ytdlpVideoInfo` and the streaming invocation in `playNext()`, the bot forces `--extractor-args youtube:player_client=ios,web_embedded,mweb,tv;formats=missing_pot` to bypass GVS PO Token enforcement. To keep playback reliable, these calls run **without** cookies by default so that the fast `ios` client is used. If a playback or lookup fails (due to age-restrictions or bot-blocking), the bot automatically retries **with** cookies; on retry, it skips `ios` and falls back to `web_embedded`, `mweb`, or `tv` using the container's **Deno** runtime and pip-installed `yt-dlp` EJS scripts to solve the JavaScript n-challenge, successfully bypassing PO Token requirements. `play-dl` is retained only for Spotify URL parsing (`play.spotify()`).

> [!WARNING]
> **Minimum yt-dlp version**: The bot passes `--js-runtimes` and `--remote-components`, which require **yt-dlp >= 2025.09.26**. If your `yt-dlp` predates this, playback will fail immediately with `yt-dlp: error: no such option: --js-runtimes` while `/find` still appears to work (it uses a separate, simpler code path). The bot detects this automatically at startup and falls back to legacy args with a console warning, but you should still keep yt-dlp current — run `pip install -U yt-dlp` manually, or rebuild the Docker image, since the `Dockerfile` now always installs the latest version on build (`pip3 install --upgrade`) rather than relying on a cached layer.