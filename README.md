...
> [!NOTE]
> All YouTube interactions — including **search, metadata lookups, and audio streaming** — are routed through `yt-dlp`. `--extractor-args "youtube:player_skip=webpage,configs"` is applied **only** to `ytdlpSearch` (flat-playlist search) where no stream URL resolution is needed. For `ytdlpVideoInfo` and the streaming invocation in `playNext()`, the bot forces `--extractor-args youtube:player_client=ios;formats=missing_pot` to bypass GVS PO Token enforcement and the JavaScript n-challenge requirement entirely. Cookies are omitted from `ytdlpVideoInfo` and `playNext()` because the `ios` client does not support cookies, and passing them causes `yt-dlp` to skip the `ios` client, resulting in playback failure. `play-dl` is retained only for Spotify URL parsing (`play.spotify()`).
