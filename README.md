...
> [!NOTE]
> All YouTube interactions — including **search, metadata lookups, and audio streaming** — are routed through `yt-dlp`. `cookies.txt` is passed to every `yt-dlp` call for authenticated access. `--extractor-args "youtube:player_skip=webpage,configs"` is applied **only** to `ytdlpSearch` (flat-playlist search) where no stream URL resolution is needed; it is intentionally omitted from `ytdlpVideoInfo` (`--dump-json` resolves the formats list and requires the player config) and the streaming invocation. The streaming call instead uses `--extractor-args youtube:player_client=ios` to bypass the JavaScript n-challenge requirement entirely (the iOS client uses OAuth tokens, not nsig). `play-dl` is retained only for Spotify URL parsing (`play.spotify()`).
